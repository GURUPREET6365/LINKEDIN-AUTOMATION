...
# Point(x=224, y=328), This is the point to click to activate the job.
# Point(x=248, y=435), This is the point to click to activate the job.

# position()

def indicate_position():
    time.sleep(4)
    pyautogui.moveTo(x=224, y=435)
...
def scroll():
    ...
    if old_url == '':
        old_url = url
        print('empty url is found........')
        print(old_url)

    elif old_url == url:
        pyautogui.moveTo(x=224, y=328) 
        time.sleep(0.3)
        pyautogui.scroll(-70)
        time.sleep(1)
        pyautogui.click()
